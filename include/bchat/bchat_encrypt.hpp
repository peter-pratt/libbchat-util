...
// Helper functions for the "BChat Protocol" encryption mechanism.  This is the encryption used
// for DMs sent from one BChat user to another.
//
...
// pubkeys in hex form are bchat ids, but without the `05` prefix).
//
...
// pubkey to work out the BChat ID:
//
...
///
/// Performs bchat protocol encryption, typically for a DM sent between BChat users.
///
/// Inputs:
...
/// - `recipient_pubkey` -- the recipient X25519 pubkey, either as a 0x05-prefixed bchat ID
...
///
/// Performs bchat protocol encryption, but using a deterministic version of crypto_box_seal.
///
...
/// - signed by the user's underlying bchat Ed25519 pubkey
/// - tagged with the user's underlying bchat Ed25519 pubkey (from which the bchat id can be
///   computed).
...
///   converted to a x25519 pubkey to extract the sender's bchat id when decrypting).
...
///
/// Performs the signing steps for bchat protocol encryption.  This is responsible for producing
...
///   0x05 bchat id prefix (33 bytes if prefixed, 32 if not prefixed).
...
/// signature on the message and converts the extracted sender's Ed25519 pubkey into a bchat ID.
///
/// Inputs:
/// - `ed25519_privkey` -- the private key of the recipient.  Can be a 32-byte seed, or a 64-byte
///   libsodium secret key.  The latter is a bit faster as it doesn't have to re-compute the pubkey
...
/// encrypted and the
///   bchat ID (in hex), *if* the message decrypted and validated successfully.  Throws on error.
...
///
/// This function attempts to decrypt a message using the BChatBlindingProtocol. If the
...
///
/// Outputs:
/// - `std::pair<std::string, std::vector<unsigned char>>` -- the bchat ID (in hex) and the
/// plaintext binary
///   data that was encrypted.
///
/// On failure this throws a std::exception-derived exception with a `.what()` string containing
/// some diagnostic info on what part failed.  Typically a production bchat client would catch
...
///
/// Outputs:
/// - `std::string` -- the bchat ID (in hex) returned from the server, *if* the server returned
///   a bchat ID.  Throws on error/failure.
