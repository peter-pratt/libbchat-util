...
/// A complimentary file to bchat encrypt (which has the low level encryption function for BChat
/// protocol types). This file contains high-level helper functions for decoding payloads on the
/// BChat protocol. Prefer functions here before resorting to the lower-level cryptography.
///
/// The general overview is that this file introduces functionality to abstract away protobufs types
/// on the BChat Protocol from client implementations by wrapping the data structures where
/// necessary in libbchat. In general clients will use the functions in this file as follows:
///
/// - Derive BChat Pro feature flags from a message for populating the new Pro fields for messages
///
...
/// implementations such that the only dependency clients need to encode and decode BChat Protocol
/// messages is libbchat itself and that it will provide wrapper/proxy types for and handle
...
enum class ProStatus {
    ...
    InvalidProBackendSig = BCHAT_PROTOCOL_PRO_STATUS_INVALID_PRO_BACKEND_SIG,
    // Pro sig in the envelope was not signed by the Rotating key
    InvalidUserSig = BCHAT_PROTOCOL_PRO_STATUS_INVALID_USER_SIG,
    Valid = BCHAT_PROTOCOL_PRO_STATUS_VALID,      // Proof is verified; has not expired
    Expired = BCHAT_PROTOCOL_PRO_STATUS_EXPIRED,  // Proof is verified; has expired
};
...
class ProProof {
  public:
    /// Version of the proof set by the BChat Pro Backend
    std::uint8_t version;

    /// Hash of the generation index set by the BChat Pro Backend
    array_uc32 gen_index_hash;

    /// The public key that the BChat client registers their BChat Pro entitlement under.
    /// BChat clients must sign messages with this key along side the sending of this proof for
    /// the network to authenticate their usage of the proof
    array_uc32 rotating_pubkey;

    /// Unix epoch timestamp to which this proof's entitlement to BChat Pro features is valid to
    sys_ms expiry_unix_ts;

    /// Signature over the contents of the proof. It is signed by the BChat Pro Backend key which
    /// is the entity responsible for issueing tamper-proof Sesison Pro certificates for BChat
    /// clients.
    array_uc64 sig;
    ...
    ///
    /// For BChat Pro intents and purposes, we expect proofs to be signed by the BChat Pro
...
enum class ProFeaturesForMsgStatus {
    Success = BCHAT_PROTOCOL_PRO_FEATURES_FOR_MSG_STATUS_SUCCESS,

    /// Message byte stream to classify could not be decoded into a valid UTF8/16 string
    UTFDecodingError = BCHAT_PROTOCOL_PRO_FEATURES_FOR_MSG_STATUS_UTF_DECODING_ERROR,

    /// Decoded UTF8/16 string exceeded the maximum character limit allowed for BChat Pro
    ExceedsCharacterLimit = BCHAT_PROTOCOL_PRO_FEATURES_FOR_MSG_STATUS_EXCEEDS_CHARACTER_LIMIT,
};

struct ProProfileBitset {
    uint64_t data;
    void set(BCHAT_PROTOCOL_PRO_PROFILE_FEATURES features);
    void unset(BCHAT_PROTOCOL_PRO_PROFILE_FEATURES features);
    bool is_set(BCHAT_PROTOCOL_PRO_PROFILE_FEATURES features) const;
};

struct ProMessageBitset {
    uint64_t data;
    void set(BCHAT_PROTOCOL_PRO_MESSAGE_FEATURES features);
    void unset(BCHAT_PROTOCOL_PRO_MESSAGE_FEATURES features);
    bool is_set(BCHAT_PROTOCOL_PRO_MESSAGE_FEATURES features) const;
};
...
enum class DestinationType {
    SyncOr1o1 = BCHAT_PROTOCOL_DESTINATION_TYPE_SYNC_OR_1O1,
    ...
    /// Destination.
    Group = BCHAT_PROTOCOL_DESTINATION_TYPE_GROUP,
    CommunityInbox = BCHAT_PROTOCOL_DESTINATION_TYPE_COMMUNITY_INBOX,
    Community = BCHAT_PROTOCOL_DESTINATION_TYPE_COMMUNITY,
};

struct Destination {
    DestinationType type;

    // Optional rotating BChat Pro Ed25519 private key to sign the message with on behalf of the
    // caller. The BChat Pro signature must _not_ be set in the plaintext content passed into the
    // encoding function.
    ...
    // When type => (CommunityInbox || SyncMessage || Contact): set to the recipient's BChat
    // public key
...
struct DecodedPro {
    ...
    // BChat Pro proof that was embedded in the envelope, this is always populated irrespective of
...
/// API: bchat_protocol/pro_features_for_utf8
///
...
///   access to the higher character limit available in BChat Pro
///
/// Outputs:
...
/// API: bchat_protocol/pro_features_for_utf16
///
/// Determine the Pro features that are used in a given conversation message.
///
/// Inputs:
...
/// API: bchat_protocol/pad_message
///
...
/// API: bchat_protocol/encode_for_1o1
///
...
///   secret component of the user's BChat Pro Proof `rotating_pubkey`. This key is authorised to
...
/// API: bchat_protocol/encode_for_community_inbox
///
...
/// API: bchat_protocol/encode_for_community
///
...
/// API: bchat_protocol/encode_for_group
///
...
/// API: bchat_protocol/encode_for_destination
///
...
/// API: bchat_protocol/decode_envelope
///
...
///
/// If the message does not use BChat Pro features, the `pro` object will be set to nil. Otherwise
/// the pro fields will be populated with data about the BChat Pro proof embedded in the envelope
/// including the features used and if the proof was valid/expired e.t.c.
///
/// The sent timestamp of the protobuf encoded content is the timestamp that the BChat Pro proof
...
/// function does not throw if the BChat Pro proof failed to verify. Always check the pro status
/// field to verify if the BChat Pro was present and/or valid or invalid.
///
/// Inputs:
...
/// API: bchat_protocol/decode_for_community
///
...
/// Initialiser the blake2b hashing context to generate 32 byte hashes for BChat Pro features.
