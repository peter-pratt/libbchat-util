...
namespace master_node_disk_format {
...
struct master_node {
    ...
    bchat::network::x25519_pubkey swarm_pubkey() const;

    std::string to_string() const;
    std::string to_https_string() const;
    std::string to_omq_string() const;

    static master_node from(const network_master_node& node);
    void into(network_master_node& n) const;
    ...
    static master_node from_disk(std::string_view str);
    static std::pair<std::vector<master_node>, int> process_mnode_cache_bin(
            std::vector<std::byte> cache_bin);

    static master_node from_json(nlohmann::json json);

    bool operator==(const master_node& other) const = default;
    auto operator<=>(const master_node& other) const = default;
};

inline std::ostream& operator<<(std::ostream& os, const master_node& sn) {
