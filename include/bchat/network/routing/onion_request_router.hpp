...
namespace cached_edge_node_disk_format {
    ...
    constexpr size_t MAX_LINE_SIZE = master_node_disk_format::MAX_LINE_SIZE + TIMESTAMP;
}  // namespace cached_edge_node_disk_format
...
class OnionRequestRouter : public IRouter, public std::enable_shared_from_this<OnionRequestRouter> {
  ...
  public:
    ...
    std::vector<master_node> get_all_used_nodes() override;
  ...
  private:
    ...
    void _build_path(
            ...
            const std::vector<master_node>& nodes_to_exclude,
            std::optional<std::string> original_path_id = std::nullopt,
            std::optional<master_node> specific_edge_node = std::nullopt,
