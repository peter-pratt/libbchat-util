...
/// API: crypto/bchat_ed25519_key_pair
///
...
/// Outputs:
/// - `bool` -- True if the seed was successfully retrieved, false if failed.
LIBBCHAT_EXPORT bool bchat_ed25519_key_pair_seed(
        const unsigned char* ed25519_seed, /* 32 bytes */
        unsigned char* ed25519_pk_out,     /* 32 byte output buffer */
        unsigned char* ed25519_sk_out /* 64 byte output buffer */);

/// API: crypto/bchat_seed_for_ed_privkey
///
...
/// Outputs:
/// - `bool` -- True if the seed was successfully retrieved, false if failed.
LIBBCHAT_EXPORT bool bchat_ed25519_sign(
        const unsigned char* ed25519_privkey, /* 64 bytes */
        const unsigned char* msg,
        size_t msg_len,
        unsigned char* ed25519_sig_out /* 64 byte output buffer */);

/// API: crypto/bchat_ed25519_verify
///
...
LIBBCHAT_EXPORT bool bchat_ed25519_verify(
        const unsigned char* sig, /* 64 bytes */
        const unsigned char* pubkey,
        const unsigned char* msg,
        size_t msg_len);

/// API: crypto/bchat_ed25519_pro_privkey_for_ed25519_seed
///
/// Generate the deterministic Master BChat Pro key for signing requests to interact with the
/// BChat Pro features of the protocol.
///
/// Inputs:
/// - `ed25519_seed` -- [in] the seed to the long-term key for the BChat account to derive the
...
LIBBCHAT_EXPORT bool bchat_ed25519_pro_privkey_for_ed25519_seed(
        const unsigned char* ed25519_seed, /* 32 bytes */
        unsigned char* ed25519_sk_out /*64 byte output buffer*/);
