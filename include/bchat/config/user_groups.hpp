...
///     m - set of member bchat ids (each 33 bytes).
///     a - set of admin bchat ids (each 33 bytes).
...
struct legacy_group_info : base_group_info {
    std::string bchat_id;                      // The legacy group "bchat id" (33 bytes).
    ...
    /// Constructs a new legacy group info from an id (which must look like a bchat_id).  Throws
    /// if id is invalid.
    explicit legacy_group_info(std::string sid);

    /// API: user_groups/legacy_group_info::members
    ///
    /// Accesses the bchat ids (in hex) of members of this group.  The key is the hex bchat_id;
    ...
    ///
    /// Adds a member (by bchat id and admin status) to this group.  Returns true if the member
    /// was inserted or changed admin status, false if the member already existed.  Throws
    /// std::invalid_argument if the given bchat id is invalid.
    ///
    /// Inputs:
    ...
    bool insert(std::string bchat_id, bool admin);

    /// API: user_groups/legacy_group_info::erase
    ///
    /// Removes a member (by bchat id) from this group.  Returns true if the member was
    ...
    bool erase(const std::string& bchat_id);
...
struct group_info : base_group_info {
    std::string id;  // The group pubkey (66 hex digits); this is an ed25519 key, prefixed with "03"
                     // (to distinguish it from a 05 x25519 pubkey bchat id).
...
class UserGroups : public ConfigBase {
  ...
  public:
    ...
    ///
    /// Looks up and returns a legacy group by group ID (hex, looks like a BChat ID).  Returns
    /// nullopt if the group was not found, otherwise returns a filled out `legacy_group_info`.
    ///
    /// Inputs:
    /// - `pubkey_hex` -- group ID (hex, looks like a bchat ID)
    ///
    /// Outputs:
    ...
    ///
    /// Looks up and returns a group (aka new group) by group ID (hex, looks like a BChat
    /// ID but starting with 03).  Returns nullopt if the group was not found, otherwise returns a
    /// filled out `group_info`.
    ///
    /// Inputs:
    /// - `pubkey_hex` -- group ID (hex, looks like a bchat ID but starting 03 instead of 05)
    ///
    /// Outputs:
    ...
    /// you would do:
    /// ```cpp
    ///     auto info = conversations.get_or_construct_community(some_bchat_id);
  ...
  public:
    ...
    ///
    /// Inputs:
    /// - `pubkey_hex` -- group ID (hex, looks like a bchat ID, but starts with 03)
    ///
    /// Outputs:
    /// - `bool` - Returns true if found and removed, false otherwise
    ...
    ///             // use lg->bchat_id, lg->priority, etc.
    ///         }
    ///     }
    /// ```
    ///
