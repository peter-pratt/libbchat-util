...
typedef struct convo_info_volatile_1to1 {
    ...
    bytes32 pro_gen_index_hash;      // Hash of the generation index set by the BChat Pro Backend
    uint64_t pro_expiry_unix_ts_ms;  // Unix epoch timestamp to which this contacts entitlement to
                                     // BChat Pro features is valid to
...
typedef struct convo_info_volatile_legacy_group {
    char group_id[67];  // in hex; 66 hex chars + null terminator.  Looks just like a BChat ID,
...
typedef struct convo_info_volatile_blinded_1to1 {
    char blinded_bchat_id[67];  // in hex; 66 hex chars + null terminator.
...
LIBBCHAT_EXPORT int convo_info_volatile_init(
        config_object** conf,
...
///
/// Fills `convo` with the conversation info given a bchat ID (specified as a null-terminated hex
...
/// - `bchat_id` -- [in] Null terminated hex string of the bchat_id
///
/// Outputs:
/// - `bool` - Returns true if the conversation exists
LIBBCHAT_EXPORT bool convo_info_volatile_get_1to1(
        config_object* conf,
...
/// this sets all the convo fields to defaults and loads it with the given bchat_id.
///
/// Returns true as long as it is given a valid bchat_id.  A false return is considered an error,
/// and means the bchat_id was not a valid bchat_id.  In such a case `conf->last_error` will be
...
/// Outputs:
/// - `bool` - Returns true if the conversation exists
LIBBCHAT_EXPORT bool convo_info_volatile_get_or_construct_1to1(
        config_object* conf,
        convo_info_volatile_1to1* convo,
        const char* bchat_id) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool convo_info_volatile_get_community(
        config_object* conf,
        convo_info_volatile_community* comm,
        const char* base_url,
        const char* room) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool convo_info_volatile_get_or_construct_community(
        config_object* conf,
        convo_info_volatile_community* convo,
        const char* base_url,
        const char* room,
        unsigned const char* pubkey) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool convo_info_volatile_get_group(
        config_object* conf,
        convo_info_volatile_group* convo,
        const char* id) LIBBCHAT_WARN_UNUSED;
...
/// false return is considered an error, and means the id was not a valid bchat id; an error
...
/// Outputs:
/// - `bool` - Returns true if the call succeeds
LIBBCHAT_EXPORT bool convo_info_volatile_get_or_construct_group(
        config_object* conf,
        convo_info_volatile_group* convo,
        const char* id) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool convo_info_volatile_get_legacy_group(
        config_object* conf,
...
/// fields to defaults and loads it with the given id.
///
/// Returns true as long as it is given a valid legacy group id (i.e. same format as a bchat id).
/// A false return is considered an error, and means the id was not a valid bchat id; an error
/// string will be set in `conf->last_error`.
///
/// This is the method that should usually be used to create or update a conversation, followed by
/// setting fields in the convo, and then giving it to convo_info_volatile_set().
...
/// Outputs:
/// - `bool` - Returns true if the call succeeds
LIBBCHAT_EXPORT bool convo_info_volatile_get_or_construct_legacy_group(
        config_object* conf,
        convo_info_volatile_legacy_group* convo,
        const char* id) LIBBCHAT_WARN_UNUSED;
...
///
/// Fills `convo` with the conversation info given a blinded bchat ID (specified as a
...
///     [in]    const char*                         blinded_bchat_id
/// );
/// ```
///
/// Inputs:
/// - `conf` -- [in] Pointer to the config object
/// - `convo` -- [out] Pointer to conversation info
/// - `blinded_bchat_id` -- [in] Null terminated hex string of the bchat_id
///
/// Outputs:
/// - `bool` - Returns true if the conversation exists
LIBBCHAT_EXPORT bool convo_info_volatile_get_blinded_1to1(
        config_object* conf,
        convo_info_volatile_blinded_1to1* convo,
        const char* blinded_bchat_id) LIBBCHAT_WARN_UNUSED;
...
/// blinded_bchat_id.
///
/// Returns true as long as it is given a valid blinded_bchat_id.  A false return is considered an
/// error, and means the blinded_bchat_id was not a valid blinded_bchat_id.  In such a case
...
/// - `conf` -- [in] Pointer to the config object
/// - `convo` -- [out] Pointer to conversation info
/// - `blinded_bchat_id` -- [in] Null terminated hex string of the blinded bchat id
///
/// Outputs:
/// - `bool` - Returns true if the conversation exists
LIBBCHAT_EXPORT bool convo_info_volatile_get_or_construct_blinded_1to1(
        config_object* conf,
        convo_info_volatile_blinded_1to1* convo,
        const char* blinded_bchat_id) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool convo_info_volatile_set_1to1(
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool convo_info_volatile_set_community(
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool convo_info_volatile_set_group(
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool convo_info_volatile_set_legacy_group(
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool convo_info_volatile_set_blinded_1to1(
...
LIBBCHAT_EXPORT bool convo_info_volatile_erase_1to1(config_object* conf, const char* bchat_id);
...
LIBBCHAT_EXPORT bool convo_info_volatile_erase_community(
...
LIBBCHAT_EXPORT bool convo_info_volatile_erase_group(config_object* conf, const char* group_id);
...
/// Outputs:
/// - `bool` - Returns true if group was found and removed
LIBBCHAT_EXPORT bool convo_info_volatile_erase_legacy_group(
...
/// Inputs:
/// - `conf` -- [in] Pointer to the config object
/// - `blinded_bchat_id` -- [in] Null terminated hex string
///
/// Outputs:
/// - `bool` - Returns true if conversation was found and removed
LIBBCHAT_EXPORT bool convo_info_volatile_erase_blinded_1to1(
...
LIBBCHAT_EXPORT size_t convo_info_volatile_size(const config_object* conf);
...
/// Outputs:
/// - `size_t` -- number of conversations
LIBBCHAT_EXPORT size_t convo_info_volatile_size_1to1(const config_object* conf);
...
LIBBCHAT_EXPORT size_t convo_info_volatile_size_communities(const config_object* conf);
...
LIBBCHAT_EXPORT size_t convo_info_volatile_size_groups(const config_object* conf);
...
LIBBCHAT_EXPORT size_t convo_info_volatile_size_legacy_groups(const config_object* conf);
...
/// Outputs:
/// - `size_t` -- number of conversations
LIBBCHAT_EXPORT size_t convo_info_volatile_size_blinded_1to1(const config_object* conf);
...
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new(
...
/// Outputs:
/// - `convo_info_volatile_iterator*` -- Iterator
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new_1to1(
        const config_object* conf);
...
/// Outputs:
/// - `convo_info_volatile_iterator*` -- Iterator
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new_communities(
        const config_object* conf);
...
/// Outputs:
/// - `convo_info_volatile_iterator*` -- Iterator
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new_groups(
        const config_object* conf);
...
/// Outputs:
/// - `convo_info_volatile_iterator*` -- Iterator
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new_legacy_groups(
        const config_object* conf);
...
/// Outputs:
/// - `convo_info_volatile_iterator*` -- Iterator
LIBBCHAT_EXPORT convo_info_volatile_iterator* convo_info_volatile_iterator_new_blinded_1to1(
        const config_object* conf);
...
LIBBCHAT_EXPORT void convo_info_volatile_iterator_free(convo_info_volatile_iterator* it);
...
LIBBCHAT_EXPORT bool convo_info_volatile_iterator_done(convo_info_volatile_iterator* it);
...
/// Inputs:
/// - `it` -- [in] The convo_info_volatile_iterator
LIBBCHAT_EXPORT void convo_info_volatile_iterator_advance(convo_info_volatile_iterator* it);
...
LIBBCHAT_EXPORT bool convo_info_volatile_it_is_1to1(
...
LIBBCHAT_EXPORT bool convo_info_volatile_it_is_community(
...
LIBBCHAT_EXPORT bool convo_info_volatile_it_is_group(
...
LIBBCHAT_EXPORT bool convo_info_volatile_it_is_legacy_group(
...
LIBBCHAT_EXPORT bool convo_info_volatile_it_is_blinded_1to1(
