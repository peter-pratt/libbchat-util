...
LIBBCHAT_EXPORT int user_profile_init(
        config_object** conf,
...
/// no name
LIBBCHAT_EXPORT const char* user_profile_get_name(const config_object* conf);
...
LIBBCHAT_EXPORT int user_profile_set_name(config_object* conf, const char* name);
...
LIBBCHAT_EXPORT user_profile_pic user_profile_get_pic(const config_object* conf);
...
/// Outputs:
/// - `int` -- Returns 0 on success, non-zero on error
LIBBCHAT_EXPORT int user_profile_set_pic(config_object* conf, user_profile_pic pic);
...
/// Outputs:
/// - `int` -- Returns 0 on success, non-zero on error
LIBBCHAT_EXPORT int user_profile_set_reupload_pic(config_object* conf, user_profile_pic pic);
...
LIBBCHAT_EXPORT int user_profile_get_nts_priority(const config_object* conf);
...
LIBBCHAT_EXPORT void user_profile_set_nts_priority(config_object* conf, int priority);
...
LIBBCHAT_EXPORT int user_profile_get_nts_expiry(const config_object* conf);
...
LIBBCHAT_EXPORT void user_profile_set_nts_expiry(config_object* conf, int expiry);
...
LIBBCHAT_EXPORT int user_profile_get_blinded_msgreqs(const config_object* conf);
...
/// Outputs:
/// - `void` -- Returns Nothing
LIBBCHAT_EXPORT void user_profile_set_blinded_msgreqs(config_object* conf, int enabled);
...
LIBBCHAT_EXPORT int64_t user_profile_get_profile_updated(config_object* conf);
...
LIBBCHAT_EXPORT bool user_profile_get_pro_config(const config_object* conf, pro_pro_config* pro);
...
/// Outputs:
/// - `void` -- Returns nothing
LIBBCHAT_EXPORT void user_profile_set_pro_config(config_object* conf, const pro_pro_config* pro);
...
///
/// Outputs:
/// - `bool` - A flag indicating whether the config had BChat Pro components which were removed.
LIBBCHAT_EXPORT bool user_profile_remove_pro_config(config_object* conf);
...
///
/// Outputs:
/// - `bchat_protocol_pro_profile_bitset` - bitset indicating which profile features are enabled.
LIBBCHAT_EXPORT bchat_protocol_pro_profile_bitset
...
/// profile or not.
LIBBCHAT_EXPORT void user_profile_set_pro_badge(config_object* conf, bool enabled);
...
LIBBCHAT_EXPORT void user_profile_set_animated_avatar(config_object* conf, bool enabled);

/// API: user_profile/user_profile_get_pro_access_expiry_ms
///
/// Retrieves the BChat Pro access expiry unix timestamp if it has been set, this should generally
...
/// unset.
LIBBCHAT_EXPORT uint64_t user_profile_get_pro_access_expiry_ms(const config_object* conf);
...
/// Inputs:
/// - `conf` -- [in] Pointer to the config object
/// - `access_expiry_ts_ms` -- The timestamp that the users BChat Pro access will expire, or 0 to
/// remove the value.
LIBBCHAT_EXPORT void user_profile_set_pro_access_expiry_ms(
