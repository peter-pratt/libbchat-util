...
///
/// 1 - dict of one-to-one conversations.  Each key is the BChat ID of the contact (in hex).
...
///     looks indistinguishable from a BChat ID, but isn't really a proper BChat ID).  Values
///     are dicts with keys:
///     r - the unix timestamp (integer milliseconds) of the last-read message.  Always included,
///         but will be 0 if no messages are read.
///     u - will be present and set to 1 if this conversation is specifically marked unread.
///
/// b - outgoing blinded message request conversations.  The key is the blinded BChat ID without
...
namespace convo {
    ...
    struct base {
      ...
      protected:
        ...
        friend class bchat::config::val_loader;
        friend class bchat::config::ConvoInfoVolatile;
    ...
    struct pro_base : base {
        ...
        /// Unix epoch timestamp to which this proof's entitlement to BChat Pro features is valid
        /// to
    ...
    struct one_to_one : pro_base {
        ...
        ///
        /// Constructs an empty one_to_one from a bchat_id.  BChat ID can be either bytes (33)
        /// or hex (66).
        ///
        /// Declaration:
        /// ```cpp
        /// explicit one_to_one(std::string&& bchat_id);
        /// explicit one_to_one(std::string_view bchat_id);
        /// ```
        ///
        /// Inputs:
    ...
    struct legacy_group : base {
        std::string id;  // in hex, indistinguishable from a BChat ID

        /// API: convo_info_volatile/legacy_group::legacy_group
        ///
        /// Constructs an empty legacy_group from a quasi-bchat_id
        ///
        /// Declaration:
        /// ```cpp
        ...
        ///
        /// Inputs:
        /// - `group_id` -- hex string of group_id, similar to a bchat_id
        explicit legacy_group(std::string&& group_id);
        explicit legacy_group(std::string_view group_id);
    ...
    struct blinded_one_to_one : pro_base {
        std::string blinded_bchat_id;  // in hex
        bool legacy_blinding;

        /// API: convo_info_volatile/blinded_one_to_one::blinded_one_to_one
        ///
        /// Constructs an empty blinded_one_to_one from a blinded_bchat_id.  BChat ID can be
        /// either bytes (33) or hex (66).
        ///
        /// Declaration:
        /// ```cpp
        /// explicit blinded_one_to_one(std::string&& blinded_bchat_id);
        /// explicit blinded_one_to_one(std::string_view blinded_bchat_id);
        /// ```
        ///
        /// Inputs:
        /// - `blinded_bchat_id` -- Hex string of the blinded bchat id
        explicit blinded_one_to_one(std::string&& blinded_bchat_id);
        explicit blinded_one_to_one(std::string_view blinded_bchat_id);
...
class ConvoInfoVolatile : public ConfigBase {
  ...
  public:
    ...
    std::optional<convo::one_to_one> get_1to1(std::string_view bchat_id) const;
    ...
    ///
    /// Looks up and returns a legacy group conversation by ID.  The ID looks like a hex BChat ID,
    /// but isn't really a BChat ID.  Returns nullopt if there is no record of the group
    /// conversation.
    ///
    /// Inputs:
    /// - `pubkey_hex` -- Hex string of the legacy group BChat ID
    ///
    /// Outputs:
    ...
    /// blinded bchat ID was not found, otherwise returns a filled out
    ...
    std::optional<convo::blinded_one_to_one> get_blinded_1to1(
            std::string_view blinded_bchat_id) const;
    ...
    ///
    /// Inputs:
    /// - `bchat_id` -- Hex string BChat ID
    ///
    /// Outputs:
    /// - `convo::one_to_one` - Returns a contact
    convo::one_to_one get_or_construct_1to1(std::string_view bchat_id) const;
    ...
    ///
    /// Inputs:
    /// - `blinded_bchat_id` -- Hex string blinded BChat ID
    ///
    /// Outputs:
    ...
    ///
    /// ```cpp
    ///     auto info = conversations.get_or_construct_1to1(some_bchat_id);
  ...
  public:
    ...
    ///             // use dm->bchat_id, dm->last_read, etc.
