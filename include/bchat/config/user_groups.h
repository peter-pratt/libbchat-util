...
LIBBCHAT_EXPORT extern const size_t GROUP_NAME_MAX_LENGTH;
...
LIBBCHAT_EXPORT int user_groups_init(
        config_object** conf,
...
LIBBCHAT_EXPORT bool user_groups_get_group(
...
LIBBCHAT_EXPORT bool user_groups_get_or_construct_group(
        config_object* conf, ugroups_group_info* group, const char* group_id);
...
/// Outputs:
/// - `bool` -- Whether the function succeeded or not
LIBBCHAT_EXPORT bool user_groups_get_or_construct_community(
        config_object* conf,
...
LIBBCHAT_EXPORT ugroups_legacy_group_info* user_groups_get_legacy_group(
        config_object* conf, const char* id) LIBBCHAT_WARN_UNUSED;
...
/// format as a bchat id); it will return NULL only if the given id is invalid (and so the caller
...
/// Outputs:
/// - `ugroupts_legacy_group_info*` -- Pointer containing conversation info
LIBBCHAT_EXPORT ugroups_legacy_group_info* user_groups_get_or_construct_legacy_group(
        config_object* conf, const char* id) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT void ugroups_legacy_group_free(ugroups_legacy_group_info* group);
...
LIBBCHAT_EXPORT void user_groups_set_community(
...
LIBBCHAT_EXPORT bool user_groups_set_group(config_object* conf, const ugroups_group_info* group);
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool user_groups_set_legacy_group(
...
/// Output:
/// - `bool` -- Returns true if the call succeeds, false if an error occurs.
LIBBCHAT_EXPORT bool user_groups_set_free_legacy_group(
...
LIBBCHAT_EXPORT bool user_groups_erase_community(
...
/// Outputs:
/// - `bool` -- Returns True if conversation was found and removed
LIBBCHAT_EXPORT bool user_groups_erase_group(config_object* conf, const char* group_id);
...
/// Outputs:
/// - `bool` -- Returns True if conversation was found and removed
LIBBCHAT_EXPORT bool user_groups_erase_legacy_group(config_object* conf, const char* group_id);
...
///
LIBBCHAT_EXPORT void ugroups_group_set_invited(ugroups_group_info* group);
...
/// - `group` -- [in] pointer to the group info which we should set to kicked
///
LIBBCHAT_EXPORT void ugroups_group_set_kicked(ugroups_group_info* group);
...
///
LIBBCHAT_EXPORT bool ugroups_group_is_kicked(const ugroups_group_info* group);
...
/// - `group` -- [in] pointer to the group info which we should set to kicked
///
LIBBCHAT_EXPORT void ugroups_group_set_destroyed(ugroups_group_info* group);
...
/// - `group` -- [in] pointer to the group info to query
///
LIBBCHAT_EXPORT bool ugroups_group_is_destroyed(const ugroups_group_info* group);
...
LIBBCHAT_EXPORT ugroups_legacy_members_iterator* ugroups_legacy_members_begin(
...
/// Inputs:
/// - `it` -- [in] Iterator
/// - `bchat_id` -- [out] the bchat_id of the next member will be put here
/// - `admin` -- [out] will be true if the next member is an admin
///
/// Outputs:
/// - `bool` -- Returns False when end of group is reached
LIBBCHAT_EXPORT bool ugroups_legacy_members_next(
...
LIBBCHAT_EXPORT void ugroups_legacy_members_free(ugroups_legacy_members_iterator* it);
...
/// Inputs:
/// - `it` -- [in] The ugroups_legacy_members iterator
LIBBCHAT_EXPORT void ugroups_legacy_members_erase(ugroups_legacy_members_iterator* it);

/// API: user_groups/ugroups_legacy_members_add
///
/// Adds a member (by bchat id and admin status) to this group.  Returns true if the member was
/// inserted or had the admin status changed, false if the member already existed with the given
/// status, or if the bchat_id is not valid.
///
/// Declaration:
/// ```cpp
...
LIBBCHAT_EXPORT bool ugroups_legacy_member_remove(
        ugroups_legacy_group_info* group, const char* bchat_id);
...
LIBBCHAT_EXPORT size_t ugroups_legacy_members_count(
...
LIBBCHAT_EXPORT size_t user_groups_size(const config_object* conf);
...
/// Outputs:
/// - `size_t` -- Returns the number of conversations
LIBBCHAT_EXPORT size_t user_groups_size_communities(const config_object* conf);
...
/// Outputs:
/// - `size_t` -- Returns the number of conversations
LIBBCHAT_EXPORT size_t user_groups_size_groups(const config_object* conf);
...
/// Outputs:
/// - `size_t` -- Returns the number of conversations
LIBBCHAT_EXPORT size_t user_groups_size_legacy_groups(const config_object* conf);
...
LIBBCHAT_EXPORT user_groups_iterator* user_groups_iterator_new(const config_object* conf);
...
/// Outputs:
/// - `user_groups_iterator*` -- The Iterator
LIBBCHAT_EXPORT user_groups_iterator* user_groups_iterator_new_communities(
...
/// Outputs:
/// - `user_groups_iterator*` -- The Iterator
LIBBCHAT_EXPORT user_groups_iterator* user_groups_iterator_new_legacy_groups(
        const config_object* conf);
...
/// Outputs:
/// - `user_groups_iterator*` -- The Iterator
LIBBCHAT_EXPORT user_groups_iterator* user_groups_iterator_new_groups(const config_object* conf);
...
LIBBCHAT_EXPORT void user_groups_iterator_free(user_groups_iterator* it);
...
LIBBCHAT_EXPORT bool user_groups_iterator_done(user_groups_iterator* it);
...
/// Inputs:
/// - `it` -- [in, out] The Iterator
LIBBCHAT_EXPORT void user_groups_iterator_advance(user_groups_iterator* it);
...
LIBBCHAT_EXPORT bool user_groups_it_is_community(
...
/// group).
LIBBCHAT_EXPORT bool user_groups_it_is_group(user_groups_iterator* it, ugroups_group_info* group);
...
LIBBCHAT_EXPORT bool user_groups_it_is_legacy_group(
