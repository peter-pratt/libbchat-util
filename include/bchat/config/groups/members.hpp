...
///
/// m - dict of members; each key is the member bchat id (33 bytes), each value is a dict
/// containing subkeys:
...
struct member {
    ...
    /// API: groups/member::bchat_id
    ///
    /// Member variable
    ///
    /// The member's bchat ID, in hex.
    std::string bchat_id;
...
class Members : public ConfigBase {
  ...
  public:
    ...
    ///
    /// Looks up and returns a member by hex bchat ID.  Returns nullopt if the bchat ID was
    ...
    ///
    /// Outputs:
    /// - `std::optional<member>` - Returns nullopt if bchat ID was not found, otherwise a
    /// filled out `member` struct.
    std::optional<member> get(std::string_view pubkey_hex) const;

    /// API: groups/Members::get_or_construct
    ///
    /// Similar to get(), but if the bchat ID does not exist this returns a filled-out member
    /// containing the bchat_id (all other fields will be empty/defaulted).  This is intended to
    /// be combined with `set` to set-or-create a record.
    ///
    /// NB: calling this does *not* add the bchat id to the member list when called: that requires
    ...
    ///
    /// Outputs:
    /// - `bool` - true if that bchatid is marked as pending send locally
    ...
    member::Status get_status(const member& member) const {
        ...
        if (member.admin) {
            if (member.promotion_status == STATUS_NOT_SENT && has_pending_send(member.bchat_id))
        ...
        if (member.invite_status == STATUS_NOT_SENT && has_pending_send(member.bchat_id))
    ...
    ///
    /// Removes a bchat ID from the member list, if present.
    ///
    ...
    ///
    /// Inputs:
    /// - `bchat_id` the hex bchat ID of the member to remove
    ///
    /// Outputs:
    ...
    ///         // use member.bchat_id, member.name, etc.
    ///     }
    ///```
    ///
