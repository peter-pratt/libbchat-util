...
/// `config_free()` and similar methods from `bchat/config/base.h`; instead it must be managed by
...
LIBBCHAT_EXPORT int groups_keys_init(
...
LIBBCHAT_EXPORT void groups_keys_free(config_group_keys* conf);
...
LIBBCHAT_EXPORT size_t groups_keys_size(const config_group_keys* conf);
...
LIBBCHAT_EXPORT const unsigned char* groups_keys_get_key(const config_group_keys* conf, size_t N);
...
/// Inputs:
/// - `conf` -- the groups config object
LIBBCHAT_EXPORT const span_u8 groups_keys_group_enc_key(const config_group_keys* conf);
...
LIBBCHAT_EXPORT bool groups_keys_is_admin(const config_group_keys* conf);
...
LIBBCHAT_EXPORT bool groups_keys_load_admin_key(
        config_group_keys* conf,
...
LIBBCHAT_EXPORT bool groups_keys_rekey(
        config_group_keys* conf,
        config_object* info,
        config_object* members,
        const unsigned char** out,
        size_t* outlen) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT bool groups_keys_pending_config(
...
LIBBCHAT_EXPORT bool groups_keys_load_message(
        config_group_keys* conf,
        const char* msg_hash,
        ...
        config_object* info,
        config_object* members) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT config_string_list* groups_keys_active_hashes(const config_group_keys* conf);
...
LIBBCHAT_EXPORT bool groups_keys_needs_rekey(const config_group_keys* conf)
...
LIBBCHAT_EXPORT bool groups_keys_needs_dump(const config_group_keys* conf) LIBBCHAT_WARN_UNUSED;
...
LIBBCHAT_EXPORT void groups_keys_dump(
        config_group_keys* conf, unsigned char** out, size_t* outlen);

/// API: groups/groups_keys_key_supplement
///
/// Generates a supplemental key message for one or more bchat IDs.  This is used to distribute
...
/// - `sids` -- array of bchat IDs of the members to generate a supplemental key for; each element
///   must be an ordinary (null-terminated) C string containing the 66-character bchat id.
...
///   them on failure.
LIBBCHAT_EXPORT bool groups_keys_key_supplement(
        config_group_keys* conf,
...
/// - `bchat_id` -- the bchat ID of the member (in hex)
...
///   bchat id, or not being an admin).  If a failure occurs, sign_value will not be written to.
LIBBCHAT_EXPORT bool groups_keys_swarm_make_subaccount(
...
/// Inputs:
/// - `conf` -- the config object
/// - `bchat_id` -- the member bchat id (hex c string)
...
LIBBCHAT_EXPORT bool groups_keys_swarm_make_subaccount_flags(
        config_group_keys* conf,
        const char* bchat_id,
        bool write,
        bool del,
...
/// - `bchat_ed25519_secretkey` -- the user's BChat ID secret key (64 bytes).
...
///   the requirements.
LIBBCHAT_EXPORT bool groups_keys_swarm_verify_subaccount(
        const char* group_id,
...
LIBBCHAT_EXPORT bool groups_keys_swarm_verify_subaccount_flags(
        const char* group_id,
        const unsigned char* bchat_ed25519_secretkey,
        const unsigned char* signing_value,
        bool write,
...
LIBBCHAT_EXPORT bool groups_keys_swarm_subaccount_sign(
        config_group_keys* conf,
...
LIBBCHAT_EXPORT bool groups_keys_swarm_subaccount_sign_binary(
        config_group_keys* conf,
        const unsigned char* msg,
        size_t msg_len,
        const unsigned char* signing_value,
...
///
/// Constructs the subaccount token for a bchat id.  The main use of this is to submit a swarm
...
LIBBCHAT_EXPORT bool groups_keys_swarm_subaccount_token(
...
/// Outputs:
/// - true if the call succeeded, false if an error occured.
LIBBCHAT_EXPORT bool groups_keys_swarm_subaccount_token_flags(
        config_group_keys* conf,
        const char* bchat_id,
        bool write,
        bool del,
...
LIBBCHAT_EXPORT void groups_keys_encrypt_message(
        const config_group_keys* conf,
...
LIBBCHAT_EXPORT bool groups_keys_decrypt_message(
        config_group_keys* conf,
