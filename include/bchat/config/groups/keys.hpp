...
class Keys : public ConfigSig {
    ...
    // Returned the blinding factor for a given bchat X25519 pubkey.  This depends on the group's
  ...
  public:
    ...
    ///
    /// Inputs:
    /// - `user_ed25519_secretkey` is the ed25519 secret key backing the current user's bchat ID,
    ...
    ///
    /// Generates a supplemental key message for one or more bchat IDs.  This is used to
    ...
    ///
    /// Inputs:
    /// - `sid` or `sids` -- bchat ID(s) of the members to generate a supplemental key for (there
    ///   are two versions of this function, one taking a single ID and one taking a vector).
    ///   BChat IDs are specified in hex.
    ///
    /// Outputs:
    ...
    ///
    ///   The signing value produced will be the same (for a given `bchat_id`/`write`/`del`
    ///   values) when constructed by any admin of the group.
    std::vector<unsigned char> swarm_make_subaccount(
            std::string_view bchat_id, bool write = true, bool del = false) const;
    ...
    /// instance that takes the group id and user's bchat Ed25519 secret key as arguments; and a
    ...
    /// - `bchat_ed25519_secretkey` -- the user's BChat ID secret key.
    ...
    static bool swarm_verify_subaccount(
            std::string group_id,
            std::span<const unsigned char> bchat_ed25519_secretkey,
    ...
    ///
    /// A BChat client must process messages from the keys namespace *before* other group config
    ...
    ///
    /// Compresses, signs, and encrypts group message content with the user's underlying bchat
