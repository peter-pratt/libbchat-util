...
LIBBCHAT_EXPORT extern const size_t GROUP_INFO_NAME_MAX_LENGTH;
LIBBCHAT_EXPORT extern const size_t GROUP_INFO_DESCRIPTION_MAX_LENGTH;
...
LIBBCHAT_EXPORT int groups_info_init(
        config_object** conf,
...
/// no name
LIBBCHAT_EXPORT const char* groups_info_get_name(const config_object* conf);
...
LIBBCHAT_EXPORT int groups_info_set_name(config_object* conf, const char* name);
...
LIBBCHAT_EXPORT const char* groups_info_get_description(const config_object* conf);
...
/// Outputs:
/// - `int` -- Returns 0 on success, non-zero on error
LIBBCHAT_EXPORT int groups_info_set_description(config_object* conf, const char* description);
...
LIBBCHAT_EXPORT user_profile_pic groups_info_get_pic(const config_object* conf);
...
/// Outputs:
/// - `int` -- Returns 0 on success, non-zero on error
LIBBCHAT_EXPORT int groups_info_set_pic(config_object* conf, user_profile_pic pic);
...
LIBBCHAT_EXPORT int groups_info_get_expiry_timer(const config_object* conf);
...
LIBBCHAT_EXPORT void groups_info_set_expiry_timer(config_object* conf, int expiry);
...
LIBBCHAT_EXPORT int64_t groups_info_get_created(const config_object* conf);
...
LIBBCHAT_EXPORT void groups_info_set_created(config_object* conf, int64_t ts);
...
LIBBCHAT_EXPORT int64_t groups_info_get_delete_before(const config_object* conf);
...
/// - `conf` -- [in] Pointer to the config object
/// - `ts` -- [in] the unix timestamp, or 0 to clear a current value.
LIBBCHAT_EXPORT void groups_info_set_delete_before(config_object* conf, int64_t ts);
...
/// Outputs:
/// - `int64_t` -- Unix timestamp before which messages should be deleted.  Returns 0 if not set.
LIBBCHAT_EXPORT int64_t groups_info_get_attach_delete_before(const config_object* conf);
...
/// - `conf` -- [in] Pointer to the config object
/// - `ts` -- [in] the unix timestamp, or 0 to clear a current value.
LIBBCHAT_EXPORT void groups_info_set_attach_delete_before(config_object* conf, int64_t ts);
...
LIBBCHAT_EXPORT bool groups_info_is_destroyed(const config_object* conf);
...
/// Inputs:
/// - `conf` -- [in] Pointer to the config object
LIBBCHAT_EXPORT void groups_info_destroy_group(config_object* conf);
