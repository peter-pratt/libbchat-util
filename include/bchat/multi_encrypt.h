...
/// API: crypto/bchat_encrypt_for_multiple_simple
///
...
/// `bchat_decrypt_for_multiple_simple`.
///
/// Inputs:
...
///   These are typically binary BChat IDs, not including the 0x05 prefix.
...
///   the recipient in order to decrypt the message; unlike bchat-protocol encryption, the sender
...
///   "BChatGroupKickedMessage".  The value is arbitrary: what matters is that it is unique for
...
LIBBCHAT_EXPORT unsigned char* bchat_encrypt_for_multiple_simple(
        size_t* out_len,
...
LIBBCHAT_EXPORT unsigned char* bchat_encrypt_for_multiple_simple_ed25519(
        size_t* out_len,
        const unsigned char* const* messages,
        const size_t* message_lengths,
        size_t n_messages,
...
/// API: crypto/bchat_decrypt_for_multiple_simple
///
/// This function attempts to decrypt a message produced by `bchat_encrypt_for_multiple_simple`;
...
///   decryption succeeds.
/// - `encoded` -- the incoming message, produced by bchat_encrypt_for_multiple_simple
...
///   bchat encryption, the sender's identify is not available in the encrypted message itself.
/// - `domain` -- the encryption domain, which must be the same as the value used in
///   `bchat_encrypt_for_multiple_simple`.
///
/// Outputs:
...
LIBBCHAT_EXPORT unsigned char* bchat_decrypt_for_multiple_simple(
        size_t* out_len,
...
/// pubkey.
LIBBCHAT_EXPORT unsigned char* bchat_decrypt_for_multiple_simple_ed25519_from_x25519(
        size_t* out_len,
        const unsigned char* encoded,
        size_t encoded_len,
...
/// non-BChat ID known by an Ed25519 pubkey (03... or other non-05 keys) rather than a BChat ID.
LIBBCHAT_EXPORT unsigned char* bchat_decrypt_for_multiple_simple_ed25519(
        size_t* out_len,
        const unsigned char* encoded,
        size_t encoded_len,
        const unsigned char* ed25519_secret,
