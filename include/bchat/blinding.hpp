...
/// Helper functions for community 25xxx blinded ID from a BChat pubkey and the server pubkey.
...
///
///     k = H(BCHAT_ID || SERVER_PK) mod L,  where H = 64-byte BLAKE2b
///
/// Next we convert the BChat ID (an X25519 pubkey) to Ed25519, and also clear the sign bit
/// (i.e. take the positive).  Thus we give up one bit of the pubkey (because the initial X25519
/// -> Ed25519 conversion lost it).
///
///     X = bchat id x25519 pubkey
...
/// exposed for debugging/testing.  Takes bchat id and server pk in bytes, not hex.  bchat
...
/// Computes the two possible 15-blinded ids from a bchat id and server pubkey.  Values accepted
/// and returned are hex-encoded.
std::array<std::string, 2> blind15_id(std::string_view bchat_id, std::string_view server_pk);

/// Similar to the above, but takes the bchat id and pubkey as byte values instead of hex, and
/// returns a single 33-byte value (instead of a 66-digit hex value).  Unlike the string version,
/// bchat_id here may be passed unprefixed (i.e. 32 bytes instead of 33 with the 05 prefix).  Only
...
/// Computes the 25-blinded id from a bchat id and server pubkey.  Values accepted and
/// returned are hex-encoded.
std::string blind25_id(std::string_view bchat_id, std::string_view server_pk);

/// Same as above, but takes the bchat id and pubkey as byte values instead of hex, and returns a
/// 33-byte value (instead of a 66-digit hex value).  Unlike the string version, bchat_id here may
...
/// pubkey behind a (X25519) BChat ID.  Unlike blind15_id, knowing the true Ed25519 pubkey allows
...
///
/// If the `bchat_id` is a non-null pointer then it must point at an empty string to be populated
/// with the bchat_id associated with `ed_pubkey`.  This is here for consistency with
...
std::vector<unsigned char> blinded15_id_from_ed(
        ...
        std::vector<unsigned char>* bchat_id = nullptr);

/// Computes the 25-blinded id from a 32-byte Ed25519 pubkey, i.e. from the known underlying Ed25519
/// pubkey behind a (X25519) BChat ID.  This will be the same as blind25_id (if given the X25519
/// pubkey that the Ed25519 converts to), but is more efficient when the Ed25519 pubkey is already
/// known.
///
/// The bchat_id argument is provided to optimize input or output of the bchat ID derived from
...
/// is not present when it does a BChat ID -> Ed25519 conversion for blinding purposes.
///
/// It is recommended to pass the full 64-byte libsodium-style secret key for `ed25519_sk` (i.e.
/// seed + appended pubkey) as with just the 32-byte seed the public key has to be recomputed.
...
/// Takes in a standard bchat_id and returns a flag indicating whether it matches the given
/// blinded_id for a given server_pk.
///
/// Takes either a 15 or 25 blinded_id (66 bytes) and the server pubkey (64 bytes).
///
/// Returns a flag indicating whether the bchat_id matches the blinded_id.
