...
/// Helper functions to construct payloads to communicate with the BChat Pro Backend. The data
/// structures here are largely bindings to the endpoints exposed on the BChat Pro Backend:
///
///   https://github.com/Doy-lee/bchat-pro-backend/blob/06e82c9d5b5a0a881d12d0182358219a4081acf5/server.py#L2
///
/// The high level summary of the functionality in this file. Clients can:
///
/// 1. Build a request with `AddProPaymentRequest::build_to_json` from a BChat Pro payment and
///    submit it to the backend to register the specified Ed25519 keys for BChat Pro.
///
...
///    raw response.
///
/// 2. Attach the `ProProof` constructed from (1) into their messages. Libbchat has helper
///    functions to embed the proof into their messages via the helper functions in the BChat
...
///
///    Lastly the high-level libbchat encoding functions accept the rotating private key to which
...
///
/// 4. Query the status (and optionally payment history) of a user's BChat Pro Master Ed25519 key
...
/// TODO: Assign the BChat Pro backend public key for verifying proofs to allow users of the
...
enum struct AddProPaymentResponseStatus {
    ...
    Success = BCHAT_PRO_BACKEND_ADD_PRO_PAYMENT_RESPONSE_STATUS_SUCCESS,

    /// Backend encountered an error when attempting to claim the payment
    Error = BCHAT_PRO_BACKEND_ADD_PRO_PAYMENT_RESPONSE_STATUS_ERROR,

    /// Request JSON failed to be parsed correctly, payload was malformed or missing values
    ParseError = BCHAT_PRO_BACKEND_ADD_PRO_PAYMENT_RESPONSE_STATUS_PARSE_ERROR,

    /// Payment is already claimed
    AlreadyRedeemed = BCHAT_PRO_BACKEND_ADD_PRO_PAYMENT_RESPONSE_STATUS_ALREADY_REDEEMED,
    ...
    UnknownPayment = BCHAT_PRO_BACKEND_ADD_PRO_PAYMENT_RESPONSE_STATUS_UNKNOWN_PAYMENT,
};
...
struct ResponseHeader {
    ...
    ///  | Everything else ...| BCHAT_PRO_BACKEND_STATUS_SUCCESS or
    ///                         BCHAT_PRO_BACKEND_STATUS_ERROR
    std::uint32_t status;
...
/// Register a new BChat Pro proof to the backend. The payment is registered under the
/// `master_pkey` and authorises the `rotating_pkey` to use the proof. In practice this means that
/// the caller will receive a BChat Pro Proof that can be attached to messages that have to be
...
struct AddProPaymentRequest {
    ...
    /// 32-byte Ed25519 BChat Pro master public key derived from the BChat account seed to
    /// register a BChat Pro payment under.
    array_uc32 master_pkey;

    /// 32-byte Ed25519 BChat Pro rotating public key to authorise to use the generated BChat
    /// Pro proof
    array_uc32 rotating_pkey;

    /// Transaction containing the payment details to register on the BChat Pro backend
...
/// The generated proof from the BChat Pro backend that has been parsed from JSON. This structure
...
/// Request a new BChat Pro proof from the backend. The specified `master_pkey` must have
/// previously already registered a payment to the backend that is still active and hence entitled
/// to BChat Pro features. This endpoint can then be used to pair a new Ed25519 key to be
/// authorised to use a the BChat Pro proof.
struct GenerateProProofRequest {
    /// Request version. The latest accepted version is 0
    std::uint8_t version;

    /// 32-byte Ed25519 BChat Pro master public key to generate a BChat Pro proof from. This key
    /// must have had a prior, and still active payment registered under it for a new proof to be
    /// generated successfully.
    array_uc32 master_pkey;

    /// 32-byte Ed25519 BChat Pro rotating public key authorized to use the generated proof
    array_uc32 rotating_pkey;
...
/// Retrieve the current list of revocations for currently active BChat Pro proofs (because of
...
struct GetProRevocationsRequest {
    ...
    /// the BChat Pro Backend to omit the revocation list if it has not changed.
    std::uint32_t ticket;
...
struct GetProRevocationsResponse : public ResponseHeader {
    ...
    /// List of revoked BChat Pro proofs
...
struct ProPaymentItem {
    /// Describes the current status of the consumption of the payment for BChat Pro entitlement
    ...
    /// BChat Pro product/plan item that was purchased
    ...
    /// `BCHAT_PRO_BACKEND_PAYMENT_PROVIDER_METADATA`, provided for convenience. This pointer is
    ...
    /// the BChat Pro subscription. During the period between
    /// [expiry_unix_ts, expiry_unix_ts + grace_period_duration_ms] the user continues to have
    /// entitlement to BChat Pro. This value is only applicable if `auto_renewing` is `true`.
    ...
    /// provider. Thereafter the user must initiate a refund manually via BChat support.
...
struct GetProDetailsResponse : public ResponseHeader {
    ...
    /// Current BChat Pro entitlement status for the master public key
    BCHAT_PRO_BACKEND_USER_PRO_STATUS user_status;

    /// Error code that indicates that the BChat Pro Backend encountered an error book-keeping
    /// BChat Pro entitlement for the user. If this value is not `SUCCESS` implementing clients
    ...
    /// Deadline UNIX timestamp that a user is entitled to BChat Pro Proofs. The user is allowed
    /// to request a BChat Pro Proof from the Pro Backend up until this timestamp. Thereafter
    /// the user is no longer entitled to BChat Pro. This deadline includes the grace period if
    /// applicable.
    ///
    ...
    /// to BChat Pro whilst attempting to execute the billing of a BChat Pro subscription.
    ///
    /// This allows a user to maintain entitlement to BChat Pro across billing cycles by giving
