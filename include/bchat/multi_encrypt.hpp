...
///   See also bchat/util.hpp for a convenience function for converting other containers of
...
///   if a bchat id).  *NOT* Ed25519 pubkeys; conversion to X25519 may be required if that's what
///   you have.
...
///   type of container, bchat/util.hpp's `bchat::to_view_vector` is a convenient way to convert
///   compatible containers to this view vector.
/// - `recipients` -- vector of X25519 pubkeys of the recipients.  (If sending to BChat IDs, these
...
///   recipient in order to decrypt the message; unlike bchat-protocol encryption, the sender
///   identity is not included in the message.
/// - `domain` -- the encryption domain; this is a short string that uniquely identifies the
///   "domain" of encryption, such as "BChatGroupKickedMessage".  The value is arbitrary: what
...
/// - `sender_pubkey` -- the X25519 public key of the sender (32 bytes).  Note that unlike bchat
